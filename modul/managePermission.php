<?php
class managePermission extends initialDb
{
    ...
    private function deleteUserFromPerm($v)
    {
        ...
        foreach($this->inpArray as $id => $user)
        {
            ...
            {
                $this->log(0,"[".__METHOD__."] FOUND USER (".$v['idu']."), UNSET FROM INPUT ARRAY");
                /* UNSET */
                UNSET($this->inpArray[$id]);
                $found=true;
                break;
        ...
        if(!$found)
        {
            $this->log(0,"[".__METHOD__."] USER (".$v['idu'].") NOT FOUND IN INPUT ARRAY => DELETE");
    ...
    private function checkUserExist($idUser=0)
    {
        ...
        $this->logMulti(0,$userData);
        $this->log(0,"COUNT => ".$count);
        if($count===0)
        {
            return false;
        }
        $wsku=intval($userData[0]['wsk_u'],10);
        $this->log(0,"INTVAL WSK_U => ".$wsku);
        if($wsku!==0)
        {
            return false;
        }
    ...
    {
        $permData=$this->query('SELECT `id` FROM `uzytkownik` WHERE `id`=?',$this->idPerm);
    ...
    public function getAllPerm()
    {
        ...
        $this->query('SELECT `ID` as \'i\',`SKROT` as \'s\',`NAZWA` as \'n\',`OPIS` as \'o\' FROM `uprawnienia` WHERE ID LIKE (?) OR NAZWA LIKE (?) OR SKROT LIKE (?) OR OPIS LIKE (?) ORDER BY ID asc'
                ,$f.",".$f.",".$f.",".$f);
