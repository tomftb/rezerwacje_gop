...
--
-- Host: 127.0.0.1
-- Czas generowania: 08 Sty 2019, 07:53
...
CREATE ALGORITHM=UNDEFINED DEFINER=`rezerwacjegop`@`localhost` SQL SECURITY DEFINER VIEW `v_udzial_procent_uzyt`  AS  select `pro_i_uzyt`.`id_uzytkownik` AS `idUzytkownik`,count(`pro_i_uzyt`.`udzial_procent`) AS `procentowyUdział` from `pro_i_uzyt` where (`pro_i_uzyt`.`wsk_u` = '0') group by `pro_i_uzyt`.`id_uzytkownik` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`rezerwacjegop`@`localhost` SQL SECURITY DEFINER VIEW `v_udzial_uzyt_proj`  AS  select `pro_i_uzyt`.`id_uzytkownik` AS `idUzytkownik`,`pro_i_uzyt`.`id_projekt` AS `idProjekt` from `pro_i_uzyt` where (`pro_i_uzyt`.`wsk_u` = '0') order by `pro_i_uzyt`.`id_uzytkownik` ;
