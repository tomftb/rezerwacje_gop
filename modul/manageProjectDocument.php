...
class manageProjectDocument extends initialDb {
    //put your code here
    ...
    private function getDbDok($idProject='')
    {
         ...
         */
        $this->log(0,"[".__METHOD__."] ID PROJECT => ".$idProject);
        ...
        $this->logMultidimensional(0,$this->dbDok,__METHOD__." dbDok"); 
        return ($this->dbDok);
    }
    ...
    {
        $this->log(0,"[".__METHOD__."] $k , $v");
        if($this->getError()) { return '';}
        foreach($this->dbDok as $id => $dok)
        {
            $this->log(0,"[".__METHOD__."] $id , ".$dok['Nazwa']);
            if($v===$dok['Nazwa'])
            {
                $this->log(0,"[".__METHOD__."] FOUND => UNSET => ".$dok['Nazwa']);
    ...
    public function addDok($idProject,$dokArray)
    {
    ...
    {
        $this->log(0,"[".__METHOD__."] INSERT INTO DB => ".$dok);
        if($this->getError()) { return '';}
        $this->response['status']=1;
        $this->query('INSERT INTO `projekt_dok` (id_projekt,nazwa,mod_data,mod_user,mod_user_id,mod_host) VALUES (?,?,?,?,?,?)',$this->idProject.','.$dok.",".$this->cDT.",".$_SESSION["username"].",".$_SESSION["userid"].",".$this->RA);     
        self::setupActDokListField($this->addLabel.$dok);
    }
    
    private function removeNotSendedDoc($dok)
    {
        $this->log(0,"[".__METHOD__."] SET WSK_U = 1 FOR ".$dok['ID']." => ".$dok['Nazwa']);
        if($this->getError()) { return '';}
        /*
         * FOR TEST
         */
    ...
    private function setResponse()
    {
        ...
        $this->log(0,"[".__METHOD__."] RETURN STATUS => ".$this->response['status']);
        $this->log(0,"[".__METHOD__."] RETURN INFO => ".$this->response['info']);
