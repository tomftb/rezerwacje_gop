...
class manageRole extends initialDb
{
    ...
    public function getAllRole()
    {
        ...
        $this->query('SELECT `ID` as \'i\',`Nazwa` as \'n\' FROM `v_slo_rola_all` WHERE `WSK_U`=? AND (`ID` LIKE (?) OR `Nazwa` LIKE (?)) ORDER BY ID ASC'
                ,"0,".$f.",".$f);
        $this->logMulti(2,$this->queryReturnValue());
    ...
    public function rDelete()
    {
        ...
        $this->log(0,'id role => '.$this->idRole);
        if(!$this->idRole)
        {
    ...
    {
        if($this->response->getError()!=='') {return false;}
        $this->log(0,"[".__METHOD__."] ID ROLE => ".$idRole);
    ...
    private function removeRolePerm($idRole)
    {
        ...
        {
            $this->log(0,"[".__METHOD__."] ID PERM => ".$v['iu']);
            if(!array_key_exists('perm_'.$v['iu'], $this->inpArray ))
            {
                $this->log(0,__METHOD__." REMOVE ROLE (${idRole}) PERM (".$v['iu'].")");
    ...
    private function checkRolePermKey($id,$value)
    {
        ...
        {
            $this->log(0,"[".__METHOD__."] PERM KEY => ".$id." AND VALUE 1");
            return true;
        }
        $this->log(0,"[".__METHOD__."] NO PERM KEY, FOUND => ".$id." OR VALUE NOT 1 => ".$value);
        /* KEY perm_ NOT FOUND */
    ...
    protected function insertRolePerm($idRole,$idPerm,$value)
    {
        ...
        $this->log(0,"[".__METHOD__."] ADD ROLE (${idRole}) PERM (".$tmp_id[1].")");
        
        if(!$this->checkExistInDb('upr_i_slo_rola','id_rola=? AND id_upr=?',$idRole.','.$tmp_id[1]))
        {
            // NOT EXIST -> ADD
            $this->log(0,__METHOD__." NOT EXIST => ADD");
    ...
    protected function combineSlo($slo,$sloKey,$uprRole,$sloUserKey)
    {
        ...
        foreach($slo as $id => $value)
        {
            foreach($uprRole as $key => $valueEmpl)
            {
                ...
                {
                    /* SET CHECKED */
                    $this->log(0,__METHOD__." FOUND (".$valueEmpl[$sloUserKey]." ".$value['n'].") SET CHECKED");
                    $slo[$id]['c']=1;
                    unset($uprRole[$key]);
                    break;
                }
